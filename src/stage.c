...
void Stage_DrawTexCol(Gfx_Tex *tex, const RECT *src, const RECT_FIXED *dst, fixed_t zoom, u8 cr, u8 cg, u8 cb)
{
	...
	if (tex == &stage.tex_hud0 && ((stage.stage_id == StageId_2_4) && stage.song_step <= 0))
			return;
...
static void Stage_DrawNotes()
{
	...
	for (Note *note = stage.cur_note; note->pos != 0xFFFF; note++)
	{
		...
		if (y < FIXED_DEC(-16 - SCREEN_HEIGHT2, 1))
		{
			...
			if ((stage.stage_id >= StageId_3_1 && stage.stage_id <= StageId_3_3 && note->type == NOTE_FLAG_MINE))
			   {
			    this->health -= 0x4000;
				Stage_CutVocal();
				Stage_MissNote(this);
			...
			else if (!(note->type & (bot | NOTE_FLAG_HIT)))
			{
				//Missed note
				Stage_CutVocal();
				Stage_MissNote(this);
	...
	switch (stage.state)
	{
		case StageState_Play:
		{
			...
			if (stage.mode != StageMode_2P)
			{
				...
				//Draw health bar
				RECT health_fill = {1, 0, 254 - (256 * stage.player_state[0].health / 20000), 4};
				RECT health_back = {1, 5, 254, 4};
